...
Update your `handleEvent` function to include the 2 event values specific to 1-Click Health, leveraging the [`SdkEventValues` constant](/reference/sdk/constants#sdkeventvalues):
...
There are 2 event values to handle for 1-Click Health:
...
<table>
    ...
    <tr>
        <td>`ONE_CLICK_HEALTH_MANUAL_INPUT_FORM_SUBMITTED`</td>
        <td>User submitted the health manual-input form (fullName/birthDate fallback, shown when prior data isn't sufficient to look up insurance automatically)</td>
    </tr>
</table>
