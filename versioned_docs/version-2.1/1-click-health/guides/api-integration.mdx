...
[1-Click Signup](/1-click-signup) solves (1) by letting you autofill the user's PII. 1-Click Health solves (2) and (3) by letting you autofill the user's health insurance and run an eligibility check for that insurance.
...
<table>
    ...
    <tr>
        ...
        <td>Autofill a user's insurance information <br/> and automatically run an eligibility check for it</td>
        <td>
          - PII
          - (optional) Payer
        </td>
...
**1-Click Health is _asynchronous_,** because it can take awhile to search for a user's health insurance plans across the large number of possible payers. **Response times are typically either 3-5 seconds or 25-30 seconds.** You don't receive data in the response body of `POST /1-click/health` but rather via webhook or by polling `GET /1-click/health`. See [step 3](#3-receive-outputs) for details.
...
Call [`POST /1-click/health`](/reference/api/endpoints#post-1-clickhealth) with payer and member ID:
