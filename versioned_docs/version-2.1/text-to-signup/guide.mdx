...
import OneClickVerificationEntityExampleTextToSignup from '@site/versioned_docs/version-2.1/reusables/1-click-verification-entity-example-text-to-signup.mdx'
import OneClickHealthEntityExampleSuccceeded from '@site/versioned_docs/version-2.1/reusables/1-click-health-entity-example-succeeded.mdx'
...
<CollapsibleGroup firstSectionExpanded={false}>
    ...
    <CollapsibleSection>
        ...
        **Choose when a user should be redirected:**
        ...
        <Admonition type="tip">
        If you've already integrated 1-Click Signup/Health (following the [1-Click Signup guides](/1-click-signup/guides) or [1-Click Health guides](/1-click-health/guides) [SDK Integration](/1-click-signup/guides/sdk-integration) or [API Integration](/1-click-signup/guides/api-integration) guide), the **"Text to Signup"** option is best, because it ensures you can keep your signup flow consistent for all users.
        </Admonition>

        If you choose the **"Hosted 1-Click Signup/Health"** option, we will take the user through a hosted [1-Click Signup](/1-click-signup) and/or [1-Click health](/1-click-health) flow that autofills identity and/or health insurance info and has them confirm it. You'll need to [retrieve the user's info](#retrieve-users-info) to benefit from this: see [step 3](#3-if-necessary-do-integration-work) for how.
...
:::important
Only the styling settings listed below apply to **Text to Signup**. There are other styling settings that apply to [**1-Click Signup**](/1-click-signup) (if you're using the **SDK** integration type). See [here](/1-click-signup/guides/setup#styling-settings) in the 1-Click Signup Setup guide for details.
:::
...
<CollapsibleGroup firstSectionExpanded={false}>
    ...
    <CollapsibleSection>
        ...
        If you [pass the user to an integrated 1-Click Signup flow](#pass-user-to-integrated-1-click-signup-flow) on your side, that automatically confirms the user's phone is verified. But if you _don't_ want to pass the user to such a flow, you need to separately confirm the user's phone is verified. See below for how to do this (and why it's necessary).

        <SequenceDiagramTextToSignupConfirmUsersPhoneIsVerified/>


        #### a. Parse `verificationUuid` URL parameter.

        When a user completes Text to Signup, we redirect them to your campaign's [redirect URL](#redirect-url), appending a URL parameter that depends on the [redirect user after](#redirect-user-after) setting. When it's set to **"Text to Signup"**, we redirect the user after we verify their phone number and append a `verificationUuid` parameter, which lets you check that the user's phone was verified:
        ...
        **A user arriving at your [redirect URL](#redirect-url) with a `verificationUuid` URL parameter is NOT a technical guarantee that the user's phone is verified!** If your use case needs such a guarantee, you must confirm the phone is verified using [step b](#b-call-get-1-clickverifications) and [step c](#c-check-verification-status). 
        ...
        ```
        <OneClickVerificationEntityExampleTextToSignup/>
    ...
    <CollapsibleSection>
        ...
        <Admonition type="note">
            This integration step is only relevant if the [redirect user after](#redirect-user-after) setting is set to **"Hosted 1-Click Signup/Health".**
        </Admonition>

        <SequenceDiagramTextToSignupRetrieveUsersIdentityInfo/>

        When a user completes Text to Signup, we redirect them to your campaign's [redirect URL](#redirect-url), appending one or more URL parameters that depend on the [redirect user after](#redirect-user-after) setting. When it's set to **"Hosted 1-Click Signup/Health"**, we take the user through a hosted 1-Click Signup and/or Health flow that autofills identity and/or health insurance info and has them confirm it. In that case, we append an `identityUuid` and/or `healthDataUuid` parameter, which let(s) you retrieve data the user shared.
        ...
        Parse the value of the `identityUuid` URL parameter"
        ...
        ##### a. Parse the `healthDataUuid` URL parameter.

        Parse the value of the `healthDataUuid` URL parameter"

        ```
        {redirectUrl}?healthDataUuid={healthDataUuid}

        https://hooli.com/verified/text-to-signup?healthDataUuid=f23812ce-9337-49f9-a96c-77fda4f8247b
        ```

        <Admonition type="note" title="Sandbox Environment">
        In the [Sandbox](/reference/api/environments#sandbox) environment, we also append an `env=sandbox` parameter (for example `...?healthDataUuid={healthDataUuid}&env=sandbox`). This lets you tell test requests apart from live ones when you use the same redirect URL in both environments.
        </Admonition>

        Parse the value of this parameter, which you'll use in the next step. 


        ##### b. Call `GET /1-click/health`.

        Call your server with `healthDataUuid`, which you parsed in the previous step. The server should use your Verified API key to call [`GET /1-click/health/{healthDataUuid}`](/reference/api/endpoints#get-1-clickhealth):
        ...
        <Admonition type="note">
        The endpoint you should use here is **`GET /1-click/health`**, which is different than `GET /1-click/verifications`.
        </Admonition>

        <ApiKeysAdmonition/>

        The response will be a [`1ClickHealthEntity`](/reference/api/types#1clickhealthentity) that contains user health insurance data:
        ...
        <details>
            ...
            <OneClickHealthEntityExampleSuccceeded/>
        </details>
...
:::tip
**You can put the call to action _anywhere_ a user will have cell service!** It works equally well in both offline and online ads. For example, you can put it:
- on billboards
- on bus stop posters
- in TV commercials
- in radio ads
