...
Depending on your use case, you may need to display the user's data to them and have them confirm it's correct. You only need to display the user's [credential](/data/outputs/credentials) data (their identity data, like their address or birth date). You don't need to display any [metadata](/data/outputs/metadata), though you can choose to if it's relevant.
