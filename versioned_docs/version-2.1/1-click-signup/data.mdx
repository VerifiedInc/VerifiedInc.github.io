...
- **Birth date and/or SSN4 are only required sometimes:** if you get the [`OCE11`](./api-reference/errors#oce011) error code, which indicates that additional information is required. This happens when Verified is unable to narrow down to a single user based on only phone (and email if included).
:::
...
You can configure which risk signals you receive using the risk signals brand setting: see [here](./guides/setup#risk-signals) in the Setup guide. If Basic or Advanced risk signals are enabled, we will block users with an Overall Score greater than 600 and return a [`OCE017` error](./api-reference/errors#oce017).

See also the [`RiskSignals` type](./api-reference/types#risksignals).
...
<table>
    ...
    <tr>
        ...
        <td>
            See [`RiskSignals` example](./api-reference/types#example-risksignals)
        </td>
    </tr>
    <tr>
        <td>↳ Score</td>
        ...
        <td>Overall risk score, considering all identifiers provided. (If the score is over 600, Verified blocks the user and returns a [`OCE017` error](./api-reference/errors#oce017).)</td>
        <td>
            `463`
        </td>
    </tr>
