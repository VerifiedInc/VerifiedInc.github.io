---
id: types
title: Types
sidebar_label: Types
sidebar_position: 3
sidebar_class_name: sidebar-header-regular
description: Types used in the Verified SDK
slug: /reference/sdk/types
toc_max_heading_level: 4
---

## `SdkResult`

```typescript title="SdkResult Type"
export type SdkStep =
  | 'consent'
  | 'autoPhone'
  | 'phone'
  | 'verificationCode'
  | 'birthday'
  | 'ssn4'
  | 'fullName.firstName'
  | 'info'
  | 'healthManualInput'
  | 'healthDataLoading'
  | 'healthInfo';

export type SdkResultData = {
  healthDataUuid: string | null;
  identityUuid: string | null;
  verificationUuid: string | null;
  redirectUrl: string | null;
  birthDate: string | null;
  birthDateMismatched: boolean | null;
  phone: string | null;
  ssn4: string | null;
  ssn4Mismatched: boolean | null;
  fullName: {
    firstName: string | null;
  } | null;
  fullNameMismatched: boolean | null;
  step: SdkStep;
};

type SdkResultDataNullable = Nullable<SdkResultData>;

type SdkResultUserOptedOut =
  | (SdkResultDataNullable & {
      type: typeof SdkResultValues.USER_OPTED_OUT;
      step: 'phone';
    })
  | (SdkResultData & {
      type: typeof SdkResultValues.USER_OPTED_OUT;
      step: Exclude<SdkStep, 'phone'>;
    });

type SdkResultNoCredentialsFound = {
  type: typeof SdkResultValues.NO_CREDENTIALS_FOUND;
} & SdkResultData;

type SdkResultNoInsuranceFound = {
  type: typeof SdkResultValues.NO_INSURANCE_FOUND;
} & SdkResultData;

type SdkResultRiskScoreTooHigh = {
  type: typeof SdkResultValues.RISK_SCORE_TOO_HIGH;
} & SdkResultData;

type SdkResultMaxInputsAttemptsExceeded = {
  type: typeof SdkResultValues.MAX_INPUT_ATTEMPTS_EXCEEDED;
} & SdkResultData;

type SdkResultMaxVerificationCodeAttemptsExceeded = {
  type: typeof SdkResultValues.MAX_VERIFICATION_CODE_ATTEMPTS_EXCEEDED;
} & SdkResultData;

export type SdkResultUserSharedCredentials = {
  type: typeof SdkResultValues.USER_SHARED_CREDENTIALS;
  identityUuid: string;
} & SdkResultData;

export type SdkResultUserSharedHealthData = {
  type: typeof SdkResultValues.USER_SHARED_HEALTH_DATA;
  healthDataUuid: string;
} & SdkResultData;

export type SdkResultUserPhoneVerified = {
  type: typeof SdkResultValues.USER_PHONE_VERIFIED;
  verificationUuid: string;
} & SdkResultData;

export type SdkResult =
  | SdkResultUserOptedOut
  | SdkResultNoCredentialsFound
  | SdkResultNoInsuranceFound
  | SdkResultRiskScoreTooHigh
  | SdkResultMaxInputsAttemptsExceeded
  | SdkResultMaxVerificationCodeAttemptsExceeded
  | SdkResultUserSharedCredentials
  | SdkResultUserSharedHealthData
  | SdkResultUserPhoneVerified;
```

See the [`SdkResultValues` constant](/reference/sdk/constants#sdkresultvalues).

### Examples

```typescript title="SdkResult Example (User Phone Verified)"
{
  "type": "USER_PHONE_VERIFIED"
  "identityUuid": null,
  "healthDataUuid": null,
  "verificationUuid": "96c15f15-e726-40e7-bb2f-882a37b9f31f",
  "redirectUrl": "https://verified.inc/",
  "birthDate": null,
  "birthDateMismatched": null,
  "phone": "+12125550010",
  "ssn4": null,
  "ssn4Mismatched": null,
  "fullName": null,
  "fullNameMismatched": null,
  "step": "verificationCode",
}
```

```typescript title="SdkResult Example (User Shared Credentials)"
{
  type: 'USER_SHARED_CREDENTIALS',
  identityUuid: '3cf51925-e9fa-44f3-b091-1e1c63f4cc3d',
  redirectUrl: "https://verified.inc/?identityUuid=3cf51925-e9fa-44f3-b091-1e1c63f4cc3d",
  birthDate: "1989-08-01",
  birthDateMismatched: null,
  phone: "+12125550010",
  ssn4: "6789",
  ssn4Mismatched: null,
  step: 'info'
}
```

```typescript title="SdkResult Example (User Shared Health Data)"
{
  type: 'USER_SHARED_HEALTH_DATA',
  identityUuid: '3cf51925-e9fa-44f3-b091-1e1c63f4cc3d',
  healthDataUuid: 'c20525b0-1811-4467-92c9-56267588e600',
  redirectUrl: "https://verified.inc/?identityUuid=5635fbd7-e97f-4af4-9f1f-2769fa17aa88&healthDataUuid=c20525b0-1811-4467-92c9-56267588e600",
  birthDate: "1989-08-01",
  birthDateMismatched: null,
  phone: "+12125550010",
  ssn4: "6789",
  ssn4Mismatched: null,
  step: 'healthInfo'
}
```

```typescript title="SdkResult Example (User Opted Out)"
{
  type: 'USER_OPTED_OUT',
  identityUuid: '3cf51925-e9fa-44f3-b091-1e1c63f4cc3d',
  redirectUrl: "https://verified.inc/?identityUuid=3cf51925-e9fa-44f3-b091-1e1c63f4cc3d",
  birthDate: null,
  birthDateMismatched: null,
  phone: "+12125550010",
  ssn4: null,
  ssn4Mismatched: null,
  step: 'phone'
}
```

```typescript title="SdkResult Example (No Credentials Found)"
{
  type: 'NO_CREDENTIALS_FOUND',
  identityUuid: '3cf51925-e9fa-44f3-b091-1e1c63f4cc3d',
  redirectUrl: "https://verified.inc/?identityUuid=3cf51925-e9fa-44f3-b091-1e1c63f4cc3d",
  birthDate: "1989-08-01",
  birthDateMismatched: null,
  phone: "+12125550010",
  ssn4: "6789",
  ssn4Mismatched: null,
  step: 'ssn4'
}
```

```typescript title="SdkResult Example (No Insurance Found)"
{
  type: 'NO_INSURANCE_FOUND',
  identityUuid: '3cf51925-e9fa-44f3-b091-1e1c63f4cc3d',
  healthDataUuid: 'c20525b0-1811-4467-92c9-56267588e600',
  redirectUrl: "https://verified.inc/?identityUuid=5635fbd7-e97f-4af4-9f1f-2769fa17aa88&healthDataUuid=c20525b0-1811-4467-92c9-56267588e600",
  birthDate: "1989-08-01",
  birthDateMismatched: null,
  phone: "+12125550010",
  ssn4: "6789",
  ssn4Mismatched: null,
  step: 'ssn4'
}
```

```typescript title="SdkResult Example (Risk Score Too High)"
{
  type: 'RISK_SCORE_TOO_HIGH',
  identityUuid: '3cf51925-e9fa-44f3-b091-1e1c63f4cc3d',
  redirectUrl: "https://verified.inc/?identityUuid=3cf51925-e9fa-44f3-b091-1e1c63f4cc3d",
  birthDate: "1989-08-01",
  birthDateMismatched: null,
  phone: "+12125550010",
  ssn4: "6789",
  ssn4Mismatched: null,
  step: 'verificationCode'
}
```

```typescript title="SdkResult Example (Max Input Attempts Exceeded)"
{
  type: 'MAX_INPUT_ATTEMPTS_EXCEEDED',
  identityUuid: '3cf51925-e9fa-44f3-b091-1e1c63f4cc3d',
  redirectUrl: "https://verified.inc/?identityUuid=3cf51925-e9fa-44f3-b091-1e1c63f4cc3d",
  birthDate: "1989-08-01",
  birthDateMismatched: false,
  phone: "+12125550010",
  ssn4: "6789",
  ssn4Mismatched: true,
  step: 'ssn4'
}
```

```typescript title="SdkResult Example (Max OTP Attempts Exceeded)"
{
  type: 'MAX_VERIFICATION_CODE_ATTEMPTS_EXCEEDED',
  identityUuid: '3cf51925-e9fa-44f3-b091-1e1c63f4cc3d',
  redirectUrl: "https://verified.inc/?identityUuid=3cf51925-e9fa-44f3-b091-1e1c63f4cc3d",
  birthDate: "1989-08-01",
  birthDateMismatched: null,
  phone: "+12125550010",
  ssn4: "6789",
  ssn4Mismatched: null,
  step: 'verificationCode'
}
```

---

## `SdkError`

```typescript
type SdkError = {
  reason: typeof SdkErrorReasons;
};
```

See the [`SdkErrorReasons` constant](/reference/sdk/constants#sdkerrorreasons).

### Examples

```typescript title="SdkError Example (Session Timeout)"
{
  reason: 'SESSION_TIMEOUT';
}
```

```typescript title="SdkError Example (Invalid Session Key)"
{
  reason: 'INVALID_SESSION_KEY';
}
```

```typescript title="SdkError Example (Share Credentials Error)"
{
  reason: 'SHARE_CREDENTIALS_ERROR';
}
```

---

## `SdkEvent`

```typescript title="SdkEvent Type"
type SdkProduct = '1-click-signup' | '1-click-health' | '1-click-verify';

type SdkMetadata = {
  identityUuid: string | null;
  redirectUrl: string | null;
  birthDate: string | null;
  birthDateMismatched: boolean | null;
  phone: string | null;
  ssn4: string | null;
  ssn4Mismatched: boolean | null;
  fullName: {
    firstName: string | null;
  } | null;
  fullNameMismatched: boolean | null;
  step: SdkStep;
};

export type SdkEvent = {
  metadata: SdkMetadata;
} & (
  | { type: 'SDK_READY' }
  | { type: 'USER_STEP_CHANGE'; step: SdkStep; previousStep?: SdkStep }
  | { type: 'STEP_TIME_SPENT'; step: SdkStep; durationMs: number }
  | { type: 'USER_COMPLETED_PRODUCT'; product: SdkProduct }
  | { type: 'ONE_CLICK_SIGNUP_FORM_SUBMITTED'; form: Record<string, unknown> }
  | { type: 'ONE_CLICK_HEALTH_FORM_SUBMITTED'; form: Record<string, unknown> }
  | {
      type: 'ONE_CLICK_HEALTH_MANUAL_INPUT_FORM_SUBMITTED';
      form: {
        fullName: {
          firstName: string;
          lastName: string;
        };
        birthDate: string;
      };
    }
  | { type: 'VERIFICATION_CODE_RESENT' }
);
```

See the [`SdkEventValues` constant](/reference/sdk/constants#sdkeventvalues).

### Examples

```typescript title="SdkEvent Example (SDK Ready)"
{
  type: 'SDK_READY',
  metadata: {
    identityUuid: null,
    redirectUrl: null,
    birthDate: null,
    birthDateMismatched: null,
    phone: null,
    ssn4: null,
    ssn4Mismatched: null,
    fullName: null,
    fullNameMismatched: null,
    step: 'consent'
  }
}
```

```typescript title="SdkEvent Example (User Step Change)"
{
  type: 'USER_STEP_CHANGE',
  step: 'verificationCode',
  previousStep: 'phone',
  metadata: {
    identityUuid: null,
    redirectUrl: null,
    birthDate: null,
    birthDateMismatched: null,
    phone: '+12125550010',
    ssn4: null,
    ssn4Mismatched: null,
    fullName: null,
    fullNameMismatched: null,
    step: 'phone'
  }
}
```

```typescript title="SdkEvent Example (Step Time Spent)"
{
  type: 'STEP_TIME_SPENT',
  step: 'phone',
  durationMs: 3200,
  metadata: {
    identityUuid: null,
    redirectUrl: null,
    birthDate: null,
    birthDateMismatched: null,
    phone: '+12125550010',
    ssn4: null,
    ssn4Mismatched: null,
    fullName: null,
    fullNameMismatched: null,
    step: 'phone'
  }
}
```

```typescript title="SdkEvent Example (User Completed Product)"
{
  type: 'USER_COMPLETED_PRODUCT',
  product: '1-click-signup',
  metadata: {
    identityUuid: '3cf51925-e9fa-44f3-b091-1e1c63f4cc3d',
    redirectUrl: 'https://verified.inc/?identityUuid=3cf51925-e9fa-44f3-b091-1e1c63f4cc3d',
    birthDate: '1989-08-01',
    birthDateMismatched: null,
    phone: '+12125550010',
    ssn4: '6789',
    ssn4Mismatched: null,
    fullName: null,
    fullNameMismatched: null,
    step: 'info'
  }
}
```
