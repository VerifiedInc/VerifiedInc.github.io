...
The core functionality of our API allows your brand to participate in the Verified Network through our [1-Click Signup](/). For more advanced use cases like [Issue to Earn](/issue-to-earn) please refer to our [advanced](#advanced-functionality) API section.
...
5. (Optional) Credential request(s). _If you would like to update from the default credential requests._
...
:::caution
The same API key necessary to call [/1-click](/api-overview#1-click-onboarding) grants you, and only, you access to the encrypted credential data documented in [Get Shared Credentials](#get-shared-credentials). Please keep this key secure.
:::
...
The `url` from the response ought to then be enriched with a `redirectUrl` query param that will take the next step in your target service's user onboarding webpage. _More information on this can be found in the [1-Click Guide](/1-click-guide)._
...
The `1-click/trusted` response is identical to the [Get `1-Click` Credentials](#get-1-click-credentials) response, but with the addition of the `uuid` attribute. This `uuid` can be used to update the user's credentials if they edit them post-retrieval in your sign-up form. You are inventive in calling the update endpoint for the benefit of [issue to earn](/issue-to-earn).
...
This is an optional endpoint only relevant for the 1-click [non-hosted](/#non-hosted-variant) and [trusted](/#trusted-variants) variants. As the user data is sent directly to your application without interaction with Verified Inc's Web Wallet—part of the hosted solution—the user never gets a chance to update any pre-populated data provided. This editing and enrichment can only occur within a UI/UX that you manage and implement. This endpoint can be used to pass that information back to Verified Inc.

:::important
If this endpoint is optional why integrate it?

Your brand is incentive to call this endpoint asynchronously after the user has made edits to the data provided to you by the 1-click flows so that you qualify for the [Issue to Earn](/issue-to-earn) revenue streams.
:::
...
The response body will be credentials resembling their [credential schemas](/schema#example). Please see the [Postman](https://api.docs.verified.inc/#159ad031-30da-4e2b-afbb-c2abf15bd194) docs for examples of this response body.
...
The response body is currently in a more verbose format than that of the GET /sharedCredentials or the POST /1-click (both trusted and non-hosted) responses. However, in practice, the details of the response body are non-consequential for you as long as a 201 HTTP status code is received. Upon receipt, your brand will have the right to a revenue share the next time the user leverages those updated credential values.

---

## Advanced Functionality

The [1-Click Signup](/) functionality that is covered in the [core](#core-functionality) API section are higher level abstractions for these advanced endpoints. Thus, it is unlikely you will need to use these for most flows. However, for completeness please refer to this section for the advance use cases documentation which covers endpoints mostly related to pure [Issue to Earn](/issue-to-earn) integrations.
