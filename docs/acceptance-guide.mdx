...
import CredentialUsageExample from '../docs/reusables/credential-usage-example-snippet.mdx';
...
### Partner Acceptance
Partner Issuer Acceptance references the flow when you have a have a particular credential issuer partner in mind that will integrate more closely with your brand - ensuring the users will have the desired credentials for your brand's onboarding. This is generally the most popular integration type because of the parallels to traditional affiliate partnerships. However, this case is supercharges the traditional flow by allowing users to accept promotions with 1-click. For more information, please see the [Partner Acceptance](/acceptance-guide#partner-acceptance) section.

### General Acceptance
General Issuer Acceptance, references the implementation type where users may not be coming from a particular issuing brand. This is the flexible as it does not require any pre-arranged partnerships. However, it does require you to check whether the user has the necessary credentials for your brand's onboarding. For more information, please see the [General Acceptance](/acceptance-guide#general-acceptance) section.
...
[Partner Acceptance](/acceptance-guide#partner-acceptance) is the most streamlined flow thanks to the partner Issuer ensuring the User has the necessary credentials to perform your onboarding flow. Also, this form of integration allows for the User to interface with the Verified Inc. wallet in a pre-authenticated state, further streamlining the UX.
...
In all cases, you will need to configure a web client page to receive the `sharedCredentialsUuid` query param via a redirect from the Verified Inc. Wallet. Once the query param is retrieved it should be sent to your backend where it is used to call [`/sharedCredentials/{sharedCredentialsUuid}`](/api-overview#get-shared-credentials) to get the credentials to onboard the user. 

### Partner Acceptance

In this case, as the accepting <Tip type="brand">brand</Tip>, you only need to handle the two steps ([1](acceptance-guide#3-capture-query-param-value)) and ([2](/acceptance-guide#4-retrieve-shared-credentials-data)) below.

Another critical component of this flow is that the issuer specifies your brand `uuid` as the partner when issuing credentials to the user. This effectively ensures the user will have the necessary credentials, and it allows you to skip steps ([0.a](/acceptance-guide#0a-check-user-credentials). Because the user starts at the issuer in this type of flow, the issuer handles the redirect in step ([0.b](/acceptance-guide#0b-redirect-user-to-verified-inc-wallet)). This is why you must pre-define your desired `redirectUrl` with us. For more information, see the [partner issuance](/issuance-guide#partner-issuance) section of the issuance guide.
...
Because the issuer is not issuing credentials with your brand in mind and handling the redirect to the Verified Inc. Wallet, **you need to check whether the user has matching <Tip type="credential">credentials</Tip>.** If they do, they can share all of those credentials with 1-click.
