...
import IncludeAPoweredByGraphic from '@site/versioned_docs/version-2/reusables/include-a-powered-by-graphic.mdx'
...
| **Non-Hosted** | Hosted by You | Hosted by You | Use your own UI and your own verification code | You _do_ already verify users' phone numbers |
...
<TabItem value="hosted" label="Hosted" default>
    ...
    We recommend using a form with a phone number input that automatically submits when the user enters 10 digits. _See our [Demo](https://1-click.demo.verified.inc) for an example of this._
    ...
    *Hooli uses a phone number input, and they include a gray variant of the "powered by" graphic and Verified legal language.*
    ...
    #### a. Include a "powered by Verified" graphic.

    <IncludeAPoweredByGraphic/>
...
<TabItem value="semi-hosted" label="Semi-Hosted">
    ...
    ### 1. Prompt user for phone number and birth date.

    To start 1-Click Signup for a user (using the Semi-Hosted integration type), you'll need their phone number and birth date. (We require a birth date to ensure we retrieve data for only the correct user.)

    We recommend using a form with:
    - For the phone number, a phone number input that automatically continues when the user enters 10 digits.
    - For the birth date, a numeric input (formatted as `MM/DD/YYYY` or similar) that automatically continues when the user enters 8 digits.

    We recommend automatically submitting the form when both inputs have valid values. _See our [Demo](https://1-click.demo.verified.inc) for an example of this._
    ...
    <Admonition type="info" title="Birth Date Format">
        You'll need to pass the birth date to our API (in the [request body of `POST /1-click`](./endpoints#post-1-click-request)) in `YYYY-MM-DD` format, but you should collect it from the user in whatever form you think results in the best user experience for your use case.
    </Admonition>

    You'll also need to:
    <ol type="a">
        <li>[**Include a "powered by Verified" graphic**](#a-include-a-powered-by-verified-graphic-1)</li>
        <li>[**Include Verified legal language**](#b-include-verified-legal-language-1)</li>
    </ol>

    Here's a full example:

    ![](/img/phone-number-and-birth-date-inputs.png)
    *Hooli uses phone number and birth date inputs, and they include a gray variant of the "powered by" graphic and Verified legal language.*
    ...
    Call [`POST /1-click`](./endpoints?integrationType=semi-hosted#post-1-click) with the user's phone number and birth date.
    ...
    Call [`GET /1-click/{uuid}?{userInput}`](./endpoints?integrationType=semi-hosted#get-1-click) with `uuid` from the response body of `POST /1-click` and `userInput`.

    <UseUserInputAdmonition/>
    
    The [response body](./endpoints?integrationType=semi-hosted#get-1-click-response) will be a [`1ClickEntity`](./types#1clickentity) (see this [example](./types#example-1clickentity)), which contains the user's data and metadata about it. You can use this data however you like to create a new account for the user!

    ---


    ### 6. Prompt user to confirm their data.
...
<TabItem value="non-hosted" label="Non-Hosted">
    ...
    To start 1-Click Signup for a user (using the Non-Hosted integration type), you'll need their phone number and birth date, and you'll need to verify their phone number using your own solution.

    <Admonition type="important">
        The Non-Hosted integration type requires you to use your own phone verification code. If you'd like to use ours, switch to the [Semi-Hosted](?integrationType=semi-hosted#integration) or [Hosted](?integrationType=hosted#integration) integration type.
    </Admonition>
    ...
    ### 2. Prompt user for birth date.

    We require a birth date to ensure we retrieve data for only the correct user.

    We recommend using a form with a numeric input (formatted as `MM/DD/YYYY` or similar) that automatically submits when the user enters 8 digits.
    ...
    You'll also need to:
    <ol type="a">
        <li>[**Include a "powered by Verified" graphic**](#a-include-a-powered-by-verified-graphic-2)</li>
        <li>[**Include Verified legal language**](#b-include-verified-legal-language-2)</li>
    </ol>

    Here's a full example:

    ![](/img/birth-date-input.png)
    *Hooli uses a birth date input, and they include a gray variant of the "powered by" graphic and Verified legal language.*
    ...
    <Admonition type="tip">
        **For any phone number, in our [Sandbox](./environments#sandbox) environment, we'll return [mock data](./environments#mock-data).** You're welcome to use your own phone number(s) and/or our [test user](./environments#test-user)'s phone numbers. 
    </Admonition>

    ---


    ### 4. Prompt user to confirm their data.
