...
### TypeScript Support
The SDK is written in TypeScript and exports relevant types. Some types are pulled from our shared types library, [`@unumid/types`](https://github.com/UnumID/types). We recommend adding `@unumid/types` as a dependency to ensure full type support between this React Web SDK and the [Server SDK](/server-sdk).

## Installation

<!-- The React Web SDK is currently only available via GitHub, but will be available via the NPM and Yarn registries soon. -->
The React Web SDK is available via package managers [NPM](https://www.npmjs.com/)/[Yarn](https://yarnpkg.com/) or via the open source Github [repo](https://github.com/UnumID/web-sdk-react).
...
:::tip
An Verified Inc. <Tip type="request"/> is what a company uses to request a <Tip type="presentation"/> from a user. The request is represented in code as a `PresentationRequest` object.
:::
...
### Display Deep Links
PresentationRequests are shared to an Verified Inc.-powered mobile app a user's device via deep links. The React Web SDK determines how it displays them based on the browser's userAgent.
...
Tapping the push notification will open the corresponding app on the user's device and prompt them to share (a <Tip type="presentation"/> of) the requested <Tip type="credential">credentials</Tip>.
...
2.  upload your push notification credentials to Verified Inc. so we can send push notifications to your mobile app.

:::tip
[Contact us](mailto:support@Unum.ID) for details on how to upload your push notification credentials.
:::
...
- Determines which Verified Inc. SaaS environment the React Web SDK will connect to.

#### `apiKey`
- string
- _required_
- Your Web SDK API key (obtained from Verified Inc.).
...
- If this prop is not provided, the push notification fallback option will send the Verified Inc. SaaS's default push notification.
...
- If this prop is not provided, the SMS fallback option will send the Verified Inc. SaaS's default SMS message.
...
- If this prop is not provided, the email fallback option will send the Verified Inc. SaaS's default email message.
...
- You should provide this if you are using Verified Inc. as an additional authentication factor on top of your existing login. 
...
- Allows the [Holder App](/terminology#holder-app) to call back to your system and reference a specific user
- Example uses include associating a user in your system with the [DID](/terminology#DID) identifying them in Verified Inc. and just-in-time credential issuance.
...
Allows the [Holder App](/terminology#holder-app) to call back to your system with the user's [DID](/terminology#DID),
enabling functionality like user bootstrapping and just-in-time credential issuance (see also: [verifySubjectCredentialRequests](/server-sdk#verifysubjectcredentialrequests))
...
const App: FC = () => {
  ...
  const sendEmail = async (options: ExternalChannelMessageInput): Promise<SuccessResponse> => {
    // Call your Server SDK to send a deep link via email and return the response.
  };

  const sendSms = async (options: ExternalChannelMessageInput): Promise<SuccessResponse> => {
    // Call your Server SDK to send a deep link via SMS and return the response.
  };
...
import UnumIDWidget '@unumid/web-sdk-react';
