...
We recommend that you display the user's data using a form with autofilled inputs. _You only need to display the user's [credential](/v2/types#credentials) data (their identity data, like their address or birth date). You don't need to display any [metadata](/v2/types#metadata), though you can choose to if it's relevant._
