...
This lighweight TypeScript SDK lets your company act as one or more <Tip type="issuer">issuers</Tip> and <Tip  type="verifier">verifiers</Tip>.

**Before you begin:** You need to be registered as Unum ID customer, and you need to register at least one issuer or verifier. (You can register zero to many of each, depending on your use case.)
...
2. your issuer signing and encryption private keys
3. credential type

The first two are returned by [`registerIssuer`](#registerIssuer). The third (credential type) is part of what verifiers use to <Tip type="request"/> credentials from users. For example, Hooli FinTech might request a credential of type `KYCVerification`, issued by ACME Bank.
...
:::important
The private keys *never* leave your server. The Server SDK only needs them to perform cryptographic functions on your behalf.
:::
