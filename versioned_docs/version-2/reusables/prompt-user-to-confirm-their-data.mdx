...
<Admonition type="danger" title="Mask Social Security Numbers">
    **We require you to mask Social Security Numbers because they're particularly sensitive:**
    - Mask the first 5 digits on the server-side so that the SSN arrives at the client-side already masked.
    - If you allow the user to edit the SSN, ensure validation prevents them from only modifying the last 4 digits of the masked input.
</Admonition>
