...
<Admonition type="warning">
    **Our [test user](/v2.0/environments#test-user)'s phone numbers aren't valid phone numbers, so sending SMS to them won't work.** You can simulate the SMS step by using the [static verification code](/v2.0/environments#test-user) (`111111`), but we recommend instead using your own phone number(s) for testing, so you can send yourself real SMS messages.
</Admonition>