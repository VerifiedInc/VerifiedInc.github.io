...
:::note[Text "CREDIT" to +1 (202) 831-7415]
...
:::important[We're Soon Updating Our Data Format]

We're soon going to make an update to the format of the data we send you. This will make it much easier for you to consume!

This updated format will replace the simplified and enriched options you see below, maintaining the ease of use of the simplified option while also including helpful metadata.

<details>
    <summary>Upcoming Data Format</summary>

    **How to use:**
    - To read data:
        - **For an identifier, use `identifiers.{identifierType}`**.
            - Example: `identifiers.phone` = `"+10123456789"`
        - **For a credential, use `credentials.{shortenedCredentialType}`**.
            - Examples: 
                - `credentials.ssn` = `"123456789"`
                - `credentials.fullName.firstName` = `"Richard"`
    - To read metadata:
        - **For an identifier, use `metadata.identifiers.{metadataType}.{identifierType}`**
            - Example: `metadata.identifiers.verificationMethod.phone` = `"phone_carrier"`
        - **For a credential, use `metadata.credentials.{metadataType}.{shortenedCredentialType}`**.
            - Examples:
                - `metadata.credentials.verificationMethod.ssn` = `"phone_carrier"`
                - `metadata.credentials.verificationMethod.fullName.firstName` = `"phone_carrier"`
