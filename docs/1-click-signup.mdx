...
3. **If using `verificationOptions` containing a code, you will need to build the input for the user to submit the OTP code sent to them via SMS.** Verified Inc. is still sending the SMS on your behalf. This only needs to be handled if you are using `verificationOptions` set to `only_code` or `both_link_and_code`.
    ...
    Upon collecting the verification code from the user, you must redirect the user to the `/1-click` response body's `url` with the user-provided verification code as a query parameter. Below is an example response body when `1-click` is called with the additional non-hosted and verification options containing code attributes:
