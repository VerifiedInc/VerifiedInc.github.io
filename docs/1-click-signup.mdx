...
:::tip[Sandbox Testing]
**To test 1-Click Signup in our sandbox environment, you can use our test brand and user**:
<details>
...
The 1-Click endpoint is [`POST /1-click`](/api-overview#1-click). Call it with the user's phone number to start 1-Click Signup.
...
:::tip[You Can Define a Redirect URL as a Query Param]
When we create an API key for your brand — email us at Support@Verified.Inc if you need one — we define a redirect URL. This is where we will redirect the user after they consent (or decline) to share data with you. It's "global" to your brand in the sense that it applies to any 1-Click Signup flow you initiate.

However, if you choose redirect the user to us (i.e. implement step 2), you can also define a redirect URL as a query param, attaching it to the URL contained in the 1-Click endpoint [response](#response):

```text title="Redirect URL Query Param"
?redirectUrl=https%3A%2F%2Fexample.com
```

_Make sure to [encode](https://www.urlencoder.org) the URL, to avoid any browser issues._ 

This query param overrides any global redirect URL for your brand. 

**If you're using our [test brand](#implement), you will need to use this query param,** as our test brand doesn't have a global redirect URL for your brand!
:::
...
But you can also choose to receive an encriched response (ask us to enable this for you), which contains much more metadata.
...
<Tabs>
  <TabItem value="simplified" label="Simplified" default>
    ```typescript title="Data the User Shared"
    {
        ...
        "ssn": "123456789",
        "birthDate": "617947200" // Unix timestamp
    }
    ```
    </TabItem>
