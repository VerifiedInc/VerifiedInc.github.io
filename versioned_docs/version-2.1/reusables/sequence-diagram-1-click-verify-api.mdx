...
sequenceDiagram
    ...
    Note left of U: 2. Get channel availability.
    C ->>+ V: Call GET /1-click/verifications/channels?deviceIp={deviceIp}
    V ->>- C: Return availability
    
    Note left of U: 3. Use autofill channel.
    ...
    alt Success
        ...
        Note left of U: 4a. Prompt user for phone number.
        C ->>+ U: Show phone number field
        U ->>- C: Enter phone number

        Note left of U: 4b. Use silent channel.
        ...
        alt Success
            ...
            Note left of U: 4c. Fall back to SMS channel.

            Note left of U: 4ci. Start verification flow.
            C ->>+ V: Call POST /1-click/verifications (with SMS channel)
            V ->>- C: Return pending 1ClickVerificationEntity

            Note left of U: 4cii. Deliver verification message.
            C ->>+ V: Call POST /1-click/verifications/{uuid}/deliver
            V ->>- C: Return sending 1ClickVerificationEntity

            Note left of U: 4ciii. Finish verification flow.
            alt Code

                Note left of U: 4ciiiA. Prompt user for verification code.
                C ->>+ U: Show verification code field
                U ->>- C: Enter verification code

                Note left of U: 4ciiiB. Verify verification code.
                C ->>+ V: Call POST /1-click/verifications/{uuid}/verify
                V ->>- C: Return verified 1ClickVerificationEntity
                Note left of C: Done!
            else Link

                Note left of U: 4ciiiA. Parse URL parameter.
                C ->> C: Parse verificationUuid value.

                Note left of U: 4ciiiB. Get verification status.
