...
Before you launch 1-Click Signup live in production, you'll need to do 2 things:
1. **Add legal language to ensure compliance:** This is not necessary for your own internal testing, but it's important to include when you make 1-Click Signup available to end users in production, to ensure that users give informed consent. The legal language is:

> By using 1-Click Signup, you agree to create a Verified Inc. account and accept Verified Inc.'s [Terms of Use](https://www.verified.inc/legal#terms-of-use).

"Terms of Use" should be underlined or colored and hyperlinked to <a href="https://www.verified.inc/legal#terms-of-use">`https://www.verified.inc/legal#terms-of-use`</a>. This language should be included below the phone number input you use to start the 1-Click Signup flow.
...
To ensure you're production ready for 1-Click Signup, follow this simple checklist:
1. You've tested the 1-Click Signup flow end-to-end and it's working.
2. You've included the legal language above.
3. You're set up to send SMS to users.

Once you've checked off these items, you're ready to go live!
...
3. **If using `verificationOptions` containing a code, you will need to build the input for the user to submit the OTP code sent to them via SMS.** Verified Inc. is still sending the SMS on your behalf. This only needs to be handled if you are using `verificationOptions` set to `only_code` or `both_link_and_code`.
