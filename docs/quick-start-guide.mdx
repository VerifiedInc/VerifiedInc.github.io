...
<TabItem value="match" label="Match">
  ...
  3. **Call [`/sharedCredentials/{uuid}`](/api-overview/#get-shared-credentials)** with the `sharedCredentialsUuid` the callback URL contains. The response body will contain the credentials the user agreed to share via 1-Click IDV.
...
<TabItem value="no-match" label="No Match">
  ...
  **You must obtain user consent before issuing credentials.** Not doing so violates our terms and will prevent us from paying you for issuing credentials. 
  ...
  3. **Call [`/credentials`](/api-overview#issue-credentials))** with the user's verified email and/or phone and one or more credentials. For each credential, include a `type` and `data` in accordance with one of our [schemas](/schema). 
