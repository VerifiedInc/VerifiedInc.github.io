...
:::note Note 2
If <b>mandatory</b> or <b>required</b> fields are both missing, then the credential will be treated as optional.
:::
...
So for example, if you want to check if a user has credentials of type `SsnCredential`, `FullNameCredential` and `AddressCredential` issued by any issuer you would send the following request:
...
In the given example, the user will be asked to share their `SsnCredential`, `FullNameCredential` and `AddressCredential` credentials.

Let's breakdown to understand what is happening here:
...
> _¹The `FullNameCredential` also has a `MiddleNameCredential`, but it's `optional`._
...
- The `AddressCredential` credential is made up of 6 sub-credentials: `Line1Credential`, `Line2Credential`, `CityCredential`, `StateCredential`, `CountryCredential` and `ZipCodeCredential`.
  ...
  The user will be asked to share the `CityCredential` if available (this mean if the user has a credential of type `CityCredential` it will mandory be shared, but if the user does not have a credential of type `CityCredential` the user will still be able to share the `AddressCredential` credential).
...
Composite credentials are credentials that are made up of other credentials. For example, an `FullNameCredential` is made up of `FirstNameCredential`, `MiddleNameCredential`(optional) and `LastNameCredential`. When issuing a composite credential, you must provide the child credentials as well.
...
See the [schema](/schema) for more information.
:::
