...
> This is an optional endpoint only relevant for the 1-click [non-hosted](/#non-hosted-variant) and [trusted](/#trusted-variants) variants. Thanks to the user data being sent directly to your application without interaction with Verified Inc's Web Wallet - part of the hosted solution - the user never has a chance to update any pre-populated data provided. This means this chance to edit and enrich only happens in a UI/UX that you have to manage and implement.
...
The response body is currently in a more verbose format than the GET /sharedCredentials or the POST 1-click/trusted and POST /1-click non-hosted response. However, in practice the response body is concecential for you. As long as a 201 HTTP status code is received, your brand will have right to revenue the next time the user leverages those updates credential values.
