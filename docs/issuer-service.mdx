...
The Issuer Service is a custodial and managed interface to facilitate handling <Tip type="verifier">Issuer</Tip> operations.
...
### Setup
In order to use the Issuer Service you will need to create a Issuer in our self service [dashboard](https://dashboard.unumid.co/). *Please reference the [Quick Start](/1-click-or-free-use-case#dashboard) guide for how to use the dashboard.*
...
:::important
Unum ID only ever stores encrypted presentation data and is deleted after 5 minutes of the corresponding Issuer doing the initial data retrieval.
:::
