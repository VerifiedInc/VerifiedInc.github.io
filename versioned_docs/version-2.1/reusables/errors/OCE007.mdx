...
Instead, for `mandatory` we recommend using `if_available` or `no`. Then, if we can't source data for that credential, we'll still return data for other credentials. There's only a difference between those two options if you're using the **SDK** integration type, in which case `if_available` still marks the credential field as required, whereas `no` marks it as optional.