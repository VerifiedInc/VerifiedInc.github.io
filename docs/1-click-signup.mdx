...
sequenceDiagram
    ...
    rect rgb(200,200,200,0.25)
        ...
        opt
            note left of C: 4. (optional) Call Update 1-Click Credentials
            U-->>+C: Updates or enriches provided credential data
            C-->>V: Call /1-click/{uuid}/credentials with the user's updated data
            loop Asynchronously every time the user uses the updated credential values
...
2. **1-click credential data is return synchronously from this endpoint.**

    In the alternative `/1-click` flows the need for the OTP to be provided makes for the user's data to be passed back asynchronously via methods documented in the [receive user data](/#receive-user-data) section above.
...
sequenceDiagram
    ...
    rect rgb(200,200,200,0.25)
        ...
        opt
            note left of C: 3. (optional) Call Update 1-Click Credentials
            U-->>+C: Updates or enriches provided credential data
            C-->>V: Call /1-click/{uuid}/credentials with the user's updated data
            loop Asynchronously every time the user uses the updated credential values
                U-->>V: Uses the updated credential values
