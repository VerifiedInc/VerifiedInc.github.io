...
## Overview
This is a custodial service that is implemented with a security first architecture. It manages your keys and leverages the Server SDK to handle all cryptographic operations on your behalf. The result is a greatly simplified process of being a Verifier and getting instantaneous access to data shared by <Tip type="subject">subjects</Tip>.
...
### Setup
In order to use the Verifier Service you will need to create a Verifier in our self service [dashboard](https://dashboard.unumid.co/). *Please reference the [Quick Start](/quick-start-guide#dashboard) guide for how to use the dashboard.*
