...
**For this Quick Start Guide, just leave the default brand settings in place.** Notice that the default setting for **integration type** is Hosted. This means you'll use a user interface and phone verification that we host (see [Determine your integration type](./integration-guide#2-determine-your-integration-type) for more information). This is the quickest way to get started.
...
In a full integration, you'd send an SMS containing `code` to the user (you!) to verify the phone number. But for this Quick Start Guide, we'll skip the SMS step. Just pretend you sent the SMS.
