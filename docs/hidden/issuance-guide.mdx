...
Format the credential data according to the `properties` attribute of the credential schema. In this case, the schema is for a `SsnCredential` which can found found at [`https://schema-resolver.verified.inc/jsonSchema/SsnCredential`](https://schema-resolver.verified.inc/jsonSchema/SsnCredential) and for a `FullNameCredential` which can be found at [`https://schema-resolver.verified.inc/jsonSchema/FullNameCredential`](https://schema-resolver.verified.inc/jsonSchema/FullNameCredential).
