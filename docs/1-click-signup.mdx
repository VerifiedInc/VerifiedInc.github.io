...
To test 1-Click Signup in our sandbox environment, you can use credentials for our test brand (Hooli) and user (Richard Hendricks). _Yes, the ones from ["Silicon Valley"](https://en.wikipedia.org/wiki/Silicon_Valley_(TV_series))! 😂_
<details>
...
Ask us for an API key for your own brand by emailing us at Contact@Verified.Inc.
<details>
...
We recommend also using your own phone number, so you can receive the SMS messages (which won't work for the test user). This is necessary to test the full 1-Click Signup flow. 
...
<details>
    ...
    - whether a given credential is required or optional
