...
<TabItem value="semi-hosted" label="Semi-Hosted">
    ...
    *Hooli uses a phone number input, and they include a gray variant of the "powered by" graphic and Verified legal language.*
    ...
    - **Error:** Otherwise, the response body will contain an error indicating that we need additional information (the user's birth date) to source data. In this case, continue to [step 6](#6-if-necessary-prompt-user-for-birth-date).
    ...
    The [response body](./endpoints?integrationType=semi-hosted#patch-1-click-response) will be a [`1ClickEntity`](./types#1clickentity) (see this [example](./types#example-1clickentity)), which contains the user's data and metadata about it.
...
<TabItem value="non-hosted" label="Non-Hosted">
    ...
    *Hooli uses a phone number input, and they include a gray variant of the "powered by" graphic and Verified legal language.*
    ...
    - **Error:** Otherwise, the response body will contain an error indicating that we need additional information (the user's birth date) to source data. In this case, continue to [step 3](#3-if-necessary-prompt-user-for-birth-date).
