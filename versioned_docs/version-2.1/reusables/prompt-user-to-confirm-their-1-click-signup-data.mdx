...
<Admonition type="danger" title="Mask Social Security Numbers">
    **We require you to mask Social Security Numbers because they're particularly sensitive:**
    - **Mask the first 5 digits server side,** so the client only displays the last 4 digits. We recommend using the • character to mask, so that (for example) 000456789 is masked as ••••6789, which we recommend displaying as •••-••-6789 for extra clarity.
