...
:::important
The <Tip type="brand"/> of the credential can be _the same company_ as the one making the request! For example, a single company issues an authentication credential to a user, who later shares credentials containing that credential back with the company to authenticate.
:::

<!-- Question: Do we want to use authentication as an example here? We're not really talking about that use case any more, right?  -->
