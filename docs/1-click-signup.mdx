...
2. **1-click credential data is return synchronously by this endpoint.**
    ...
    In the alternative `/1-click` flow, you need to send the OTP back to use once you've received it from the user, and we pass back the user's data asynchronously via methods documented in the [receive user data](/#receive-user-data) section above.
