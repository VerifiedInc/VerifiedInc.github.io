...
Use these users to specifically test the `medium-low` and `medium` risk case of [risk signals metadata](./data#risk-signals). _Compare it to the [low risk (allowed)](#allowed) and [high risk (blocked)](#blocked) test users._
