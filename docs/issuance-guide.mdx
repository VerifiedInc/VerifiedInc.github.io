...
:::note
To make this an very streamlined experience for the user we include a preauthenticated JWT in the url which will automatically log them into the Verified Inc. wallet. This JWT is valid for 5 minutes and is limited in scope to only the credentials you just issued the user. 

If the user does not complete the partner onboarding process within 5 minutes they can still complete it at later point in 1-click now that they have the credentials but will required them authenticate into the Verified Inc. wallet to do so.
:::
