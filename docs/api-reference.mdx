...
6. (Optional) Redirect URL. _If you would like to redirect the user to a different URL than the one provided in the ApiKey._
...
```typescript title="1-Click Request"
{
  ...
  redirectUrl?: string // Optional: redirect url to redirect user to after the user has completed the 1-click flow. Default is the redirect url provided in the ApiKey.
}
```
