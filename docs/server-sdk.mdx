...
The first two are returned by [`registerIssuer`](#registerissuer). The third (credential types) is part of what verifiers use to <Tip type="request"/> credentials from users. For example, Hooli FinTech might request a credential of type `KYCVerification`, issued by ACME Bank. The fourth (data about the subject) can be any valid JSON.

You can optionally provide:

5. expiration date
6. decline issuing credentials to self (*not recommended*)
...
> Re-encrypt valid, non-expired credentials <Tip type="credential"/>s for a <Tip type="subject"/>.
...
:::important
It is necessary that the credentials you are wanting to re-encrypt were issued with the default value for `issueCredentialsToSelf` of [`issueCredentials`](#issuecredentials), which is `true`. It necessary that the subject was issued credentials with that setting so that you have access to re-encrypt for target subject.
:::
...
:::note
The main use case is in the `/userCredentialRequests` required Issuer endpoint defined in our [OpenAPI specification](https://gist.github.com/UnumIDAdmin/fd75b174b33ff0c5b4de87118cb11188) which servers as the interface for users to request credentials to be re-encrypted, akin to re-issued, to new devices. The credential types input, more often than not, will come directly for the credential requests
:::
...
"signingEncryptionKey": string, // your Issuer's encryption private key
