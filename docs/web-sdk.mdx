...
- **Button:** On mobile browsers, the SDK will default to displaying the deep link as a button. Tapping the button opens the corresponding app on their device. -->
...
- Information about the user (push notification identifier, phone number, and/or email).
  - __email__: The user's email address. Including this enables us to send deep links via email.
  - __phone__: The user's mobile phone number. Including this enables us to send deep links via SMS.
  - __pushToken__: An object containing a push notification identifier from the user's device as well as your push notification provider ('APNS' for Apple Push Notification Service or 'FCM' for Firebase Cloud Messaging)
