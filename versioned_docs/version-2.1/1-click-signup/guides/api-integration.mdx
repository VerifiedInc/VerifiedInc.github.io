...
- **Error:** Otherwise, the response body will contain an [error](../api-reference/errors) with a Verified error code:
    ...
    - **[`OCE013`](../api-reference/errors#oce013)** indicates that Verified found no [credentials](../data#credentials) for the user. → _Fall back to a manual signup flow._(#6-if-necessary-prompt-user-for-additional-info)._
...
- **Error:** Otherwise, the response body will contain an [error](../api-reference/errors) with a Verified error code:
    ...
    - **[`OCE013`](../api-reference/errors#oce013)** indicates that Verified found no [credentials](../data#credentials) for the user. → _Fall back to a manual signup flow._
...
We will review your integration to see that it's compliant and working properly. Once we approve it, you'll have access to a [Production](../api-reference/environments#production) API key on the Brand Details page. 

Just swap your Sandbox API key and the [Sandbox base URL](../api-reference/environments#sandbox) for a Production API key and the [Production base URL](../api-reference/environments#production), and you'll be live with 1-Click Signup!
