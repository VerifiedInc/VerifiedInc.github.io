...
## Functionality
Interfacing with the Unum ID API lets your company partcipate in the Unum ID identity data network. You can use the API methods to check if <Tip type="user">users</Tip> have credentials of interest, create <Tip type="request">requests</Tip> for and retrieve user data in the form of <Tip type="credential">credentials</Tip>. It can also be used to issue <Tip type="credential">credentials</Tip> of user data and update previously issued credential statuses, e.g. invalidate.
...
A credential requests encodes which <Tip type="credential">credentials</Tip> you're asking the user to share with your <Tip type="brand"/>. It's a list of one or more [`CredentialRequest`](https://docs.unumid.co/types/interfaces/credentialrequest.html) objects.
