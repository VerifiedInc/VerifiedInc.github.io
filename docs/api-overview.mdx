...
When a subject responds to a presentations request the verified data is sent and stored securely awaiting retrieval. Additionally as mentioned above in [Check User Credentials](#check-user-credentials), upon the user responding to the presentation request, the user will be redirected back to your client with the generated presentation's `uuid` as a query parameter.
...
#### Response Body: [`Presentation`](https://docs.unumid.co/server-sdk-typescript/interfaces/presentation.html)
