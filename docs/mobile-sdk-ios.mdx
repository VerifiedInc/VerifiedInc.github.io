...
- Privacy - Camera Usage Description ( used for QR code scanner)
    - sample: This app would like to use the camera for QR code scanning.
...
The host application can be setup to catch deeplinks under the Target -> Info tab.initialization
...
### QR Code scanning
A QR code is another way of passing deep links on a mobile device. Any QR code scanner is able to retrieve a deep link. The native phone application is capable of scanning QR codes, and there are stand along qr code scanning applications. If you have correctly set up the previous deep link section, then the SDK will catch any deep links that are received thorough QR codes.
...
### SDK Callback
Many of the SDK functions allow an optional callback method to be passed. This will allow the developer to be notified if a call succeeds, or if it encounters an error along with error details.
