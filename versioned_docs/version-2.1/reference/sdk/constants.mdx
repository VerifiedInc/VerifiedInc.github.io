...
function handleResult(data: SdkResult) {
    switch (data.type) {
        // highlight-next-line
        case SdkResultValues.USER_PHONE_VERIFIED: // Success!
            // Pass data.verificationUuid to your server (to call GET /1-click/verifications/{verificationUuid})
        // highlight-next-line
...
| `ONE_CLICK_HEALTH_MANUAL_INPUT_FORM_SUBMITTED` | 1-Click Health manual-input form was submitted (fullName/birthDate fallback) | `form` |
| `VERIFICATION_CODE_RESENT`        | User requested a new verification code        | —                       |
