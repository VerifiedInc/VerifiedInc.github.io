...
sidebar_label: Verifier Service
slug: /verifier-service
---

import TooltipWithDefaults from '../src/components/TooltipWithDefaults';
import Tip from '../src/components/Tip';

The Verifier Service is a custodial and managed interface to facilitate handling <Tip type="verifier">Verifier</Tip> operations.

## Overview
This is a custodial service that is implemented with a security first architecture to manage your keys and leverages the Server SDK to handle all cryptographic operations on your behalf. The result is a greatly simplified process of being a Verifier and getting instantaneous access to data shared by <Tip type="subject">subjects</Tip>.

:::note
If you would like to setup your Verifier integration in a non-custodial manner, please see the [Server SDK](/server-sdk) documentation.
:::

### Setup
In order to use the Verifier Service you will need to create a Verifier in our self service [dashboard](https://dashboard.verified.inc/). *Please reference the [Quick Start](/1-click-or-free-use-case#dashboard) guide for how to use the dashboard.*

### Authentication
All requests require an Authorization header token that contains your Verifier's `apiKey`. 
...
:::caution
Your Verifier ApiKey provided via the Authorization request header grants you and only you access to the encrypted data. Please keep this key secure.
:::
...
:::important
Verified Inc only ever stores encrypted presentation data and is deleted after 5 minutes of the corresponding Verifier doing the initial data retrieval.
:::
