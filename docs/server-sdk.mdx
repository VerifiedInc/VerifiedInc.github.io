...
> Handle verifying a subjectCredentialRequest and attempting to re-encrypt valid, non-expired <Tip type="credential"/>s for a <Tip type="subject"/>.
...
If the verification step fails and an error will be thrown. The response body contains all credentials that were reEncryptedSuccessfully to target subjectDid.
