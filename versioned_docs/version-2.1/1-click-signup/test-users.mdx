...
Use these users to specifically test the `low` and `medium-low` risk level cases of [risk signals](./data#risk-signals) metadata. _Compare them to the [medium risk (flagged)](#flagged) and [high risk (blocked)](#blocked) test users._
...
Use this user to specifically test the `medium` risk level case of [risk signals](./data#risk-signals) metadata. _Compare it to the [low risk (allowed)](#allowed) and [high risk (blocked)](#blocked) test users._
...
For this user, both a phone number and a birth date is needed to source [credentials](./data#credentials).
...
For this user, both a phone number and an SSN4 is needed to source [credentials](./data#credentials).
...
Use these test users test specific cases of [credential](./data#credentials) data, if they're relevant to your use case.
...
Use these users to specifically test the `medium-high` and `high` risk level cases of [risk signals](./data#risk-signals) metadata. _Compare them to the [low risk (allowed)](#allowed) and [medium risk (flagged)](#flagged) test users._
