---
id: example-app
title: 1-Click Signup Example App
sidebar_position: 3.1
sidebar_label: Example App
description: 1-Click Signup Example App
slug: /example-app
toc_max_heading_level: 2
---

**GitHub Repository:** [https://github.com/VerifiedInc/one-click-signup-example-app](https://github.com/VerifiedInc/one-click-signup-example-app)
...
export const CollapsableHeader = ({ children, isActive }) => {
    return (
      ...
      {children} {isActive ? <ExpandLess /> : <ExpandMore />}
      </div>
    );
};
...
export const CollapsableSection = ({ activeId, setActiveId, id, children }) => {
  ...
  return (
    <div style={{ marginBottom: "10px", border: "1px solid #ccc", padding: "10px" }}>
      <div onClick={() => {setActiveId(id)}} style={{ cursor: "pointer", fontWeight: "bold" }}>
        {React.cloneElement(header, { isActive })}
        
      </div>
      {isActive && <div style={{ marginTop: "10px" }}>{body}</div>}
    </div>
  );
};
...
<CollapsableGroup>
    <CollapsableSection>
         <CollapsableHeader>
## Title 1 {#my-explicit-id}
         </CollapsableHeader>
         some body content that will be collapsable
    </CollapsableSection>
    <CollapsableSection>
         <CollapsableHeader>
## Title 2
...
<h2 id="section-1">Section 1</h2>
      ...
      <h3 id="section-1-subsection-1">Subsection 1.1</h3>
