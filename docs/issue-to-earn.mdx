...
:::tip[Sandbox Testing]
**To test issuing ID cards in our sandbox environment, you can use our test brand and user**:
<details>
...
For example, if you have a verified Social Security Number (SSN) and a full name for a user, you can issue them a `SsnCredential` and `FullNameCredential`. Just call [`POST /credentials`](/api-overview#issue-credentials) with the user's email or phone, the `SsnCredential` and `FullNameCredential` type, and the SSN and full name data:
...
See our [data schema library](/schema#schema-library) for more information.
