...
Call [`POST /1-click/health`](/reference/api/endpoints#post-1-clickhealth) with the inputs you collected:
...
**You can retrieve data for 30 minutes,** at which point it's deleted. If you need to retrieve the same data again, use [`GET /1-click/health`](/reference/api/endpoints#get-1-clickhealth) instead of `POST /1-click/health` so that a duplicate 1-Click Health event isn't created (and so you aren't billed twice).
:::
...
- **Success:** If we can source health insurance data for the user, the `1ClickHealthEntity` will contain `results` with one or more health insurance plans. <br/> → _Continue to [step 4](#4-prompt-user-to-confirm)._
...
**For each health insurance plan you might use for a user, you must display the Payer Name and Member ID for the user to confirm.** These are contained in `memberId` and `payer.name`, respectively, for a given plan within the `results` array of the [`1ClickHealthEntity`](/reference/api/types#1clickhealthentity). 
