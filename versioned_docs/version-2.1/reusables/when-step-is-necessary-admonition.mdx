...
<Admonition type="note">
    This step is necessary if <a href={props.stepLink}>{props.stepName}</a> doesn't succeed, which means we need more information (the user's birth date and/or SSN4) to uniquely identify the user. The response body of `POST /1-click` will be an [error](/api-reference/errors) with error code [`OCE011`](/api-reference/errors#oce011).
