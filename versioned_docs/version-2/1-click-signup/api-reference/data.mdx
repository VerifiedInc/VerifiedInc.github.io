...
:::note
**A phone number is required to source verified data** ([credentials](#credentials) and [metadata](#metadata)) for a user. An email can't currently be used for this purpose on its own, but including it helps source better and more data — especially [risk signals](#risk-signals) metadata.
:::
