...
Use these users to specifically test the `low` and `medium-low` risk case of [risk signals metadata](./data#risk-signals). _Compare it to the [medium risk (flagged)](#flagged) and [high risk (blocked)](#blocked) test users._
...
For this user, both a phone number and birth date is needed to source [credentials](./data#credentials).
...
For this user, both a phone number and SSN4 is needed to source [credentials](./data#credentials).
...
For each of these 10 test users, you will get a [`1ClickEntity`](./types#1clickentity) with a different address. Each address is a standard address in a US state (as opposed to an address in an [overseas territory](#overseas-territory) or a [P.O. box](#po-box) address).

<TestUserDisplay
  data={{
    'Phone': '+12135550000 → +12135550009',
    'Verification Code': '111111',
    'Birth Date': '1989-08-01',
    'SSN4': '6789'
  }}
...
For this test user, you will get a [`1ClickEntity`](./types#1clickentity) with a P.O. box address.
...
Use these users to specifically test the `medium-high` and `high` risk case of [risk signals metadata](./data#risk-signals). _Compare it to the [low risk (allowed)](#allowed) and [medium risk (flagged)](#flagged) test users._

For these users, you will get the [`OCE017`](./errors#oce017) error code, indicating that Verified blocked returning data for the user because the risk score levels are `medium-high` and `high`.
