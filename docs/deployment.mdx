...
This section provides a brief overview of Unum ID [components](#components) and the deployment [process](#process). For a more technical and less conceptual overview, please see the documentation for each component (linked below).

## Components

Unum ID uses a hybrid deployment model. We provide several lightweight components that you install aand use to interact with our Identity Engine cloud:
- **[Mobile SDK](#mobile-sdk):** This works behind the scenes in your company's mobile app to securely store a user’s <Tip type="credential">credentials</Tip>. It also lets a user respond to <Tip type="request">requests</Tip> and share <Tip type="presentation">presentations</Tip> (of credentials).
  - **iOS:** written in Swift, supports Swift and Objective-C apps, 
  iOS 11.1+, iPhone 6+
  - **Android:** supports Kotlin and Java apps, API Level 23+ (Android 6.0+)
  - ~480 KB
- **[Server SDK](#server-sdk):** ​This lets you issue <Tip type="credential">credentials</Tip> to a user and verify <Tip type="presentation">presentations</Tip> (of credentials) a user sends to you.
  - **Option 1​:** Node.js SDK (contact us for other languages)
  - **Option 2​:** containerized app with exposed API
- **[Web SDK](#web-sdk):** ​This lets you display <Tip type="request">requests</Tip> to users and show the results of
verifying <Tip type="credential">credentials</Tip> they share.
...
For steps (3) and (4), we provide OpenAPI specifications and references to make this easy.

Using the Server SDK is straightforward. To issue (send) a credential to a user, you input identity data and the user’s identifier. And to request a credential from a user, you input the type of credential and the user’s identifier. The SDK handles the rest.
...
3. Define a redirect on successful credential sharing (e.g. granting the user access).

Using the Web SDK is simple. Just call the Server SDK to create requests for credentials from users. The Web SDK takes care of everything else — the user interface, fallback options, error handling, etc.
