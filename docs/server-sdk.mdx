...
:::note
Auto generated SDK documentation can be found [here](https://docs.unum.id/server-sdk-typescript/index.html). Similar documentation for object definitions can be found [here](https://docs.unum.id/types/index.html). The auto generated documentation is more granular then this page, however less styled.
:::
...
#### Response Body: [`Credential`](https://docs.unum.id/types/interfaces/credential.html) Array
...
You need to provide the credential `id` (which is [`issueCredentials`](#issuecredentials) returns per <Tip type="credential">credential</Tip>) and a [CredentialStatusOptions](https://docs.unum.id/types/modules.html#credentialstatusoptions) `status`. Currently there are only two valid statuses: "valid" and "revoked".
...
#### Response Body: [`SubjectCredentialRequestVerifiedStatus`](https://docs.unum.id/server-sdk-typescript/interfaces/subjectcredentialrequestverifiedstatus.html)
...
The [`VersionInfo`](https://docs.unum.id/types/interfaces/versioninfo.html) denotes how encrypted <a href="/terminology#presentation"><Tip type="presentation"/></a>s from Unum ID's SaaS, which originate from the Mobile SDK, should be routed to your application in order for the corresponding Server SDK version to preform verifications.
