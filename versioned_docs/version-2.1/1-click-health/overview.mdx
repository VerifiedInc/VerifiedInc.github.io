...
<table>
    ...
    <tr>
        <th>Coverage</th>
        <td>~80% of US adults (>216M people)</td>
    </tr>
    <tr>
...
**1-Click Health is the _easiest_ way to get a user's health insurance and eligibility.** From just name and birthday, Verified autofills a user's member ID, payer, and full eligibility details, so the user can onboard with insurance in less than 30 seconds.
...
- **Prevent fraud:** Stop bad actors by autofilling insurance data and using our [risk signals](/reference/data/outputs#risk-signals) metadata.

## Coverage

We can source verified health insurance data for about **80% of US adults (226M people).** The data is extremely high quality because we waterfall across a number of data provider partners like clearing houses.

<CoverageUsOnlyAdmonition />

## Integration

Integrating 1-Click Health is simple. How long it takes depends on the integration type you choose:
