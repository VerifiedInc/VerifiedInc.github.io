...
**Example:** Hooli FinTech checks if Richard has a SSN and LastName credential issued by ACME Bank. Because he does, a request is created for his SSN and LastName credential specifically from ACME Bank. Hooli presents this request to Richard by directing him to the `url` received in the `/hasMatchingCredentials` response body.
...
:::important
The <Tip type="brand"/> of the credential can be *the same company* as the one making the request! For example, a single company issues an authentication credential to a user, who later shares credentials containing that credential back with the company to authenticate.
:::
...
> A **user** is an individual in the Unum ID network that can share verified identity data (via <a href="/terminology#credential"><Tip type="credential">credentials</Tip></a>). Each user has at least one phone number or email address associated with them. They can have multiple of either.
