...
## 1. Prompt user for phone number and challenge.

To start 1-Click Signup for a user, you'll need their phone number and a [challenge](/reference/data/inputs#challenges) (first name, birthday, or SSN4).
...
To start 1-Click Signup for a user, you'll need their verified phone number. To verify the user's phone number, you can either:
1. **Use our 1-Click Verify API:** This is our dedicated phone verification solution. 
2. **Use another phone verification solution:** This can be one of the many phone verification solutions on the market or one you build yourself. You can do this however you like, as long as you verify the user's phone number with a **one-time verification code at least 6 digits long.**

:::tip We Recommend 1-Click Verify
We recommend using our 1-Click Verify API, because it ties directly into 1-Click Signup to make the integration especially easy and secure. Specifically, 1-Click Verify outputs a `verificationUuid` that you can pass directly to 1-Click Signup, which can then hydrate the user's phone number and confirm that it was properly verified.
:::

:::warning A Compliance Review is Required if You Use Another Solution
If you use another solution for phone verification, we will need to do a compliance review of your integration to ensure that phone verification is being handled properly. If you use 1-Click Verify, there's a closed loop with security guarantees, but if you use another solution, we can't have such security guarantees. So, instead we have to do a manual review of your integration to ensure phone verification is being handled properly. Specifically, we check for:
...
Regulations require that users give informed consent, so you must use our consent language where you start the 1-Click Signup flow. It can be included alongside or as part of other legal language you already use, but it must be displayed to the user — not just included in terms you link out to.
...
<TabItem value="1-click-verify" label="Use 1-Click Verify" default>

  Follow the [1-Click Verify API Integration](/1-click-verify/guides/api-integration) guide. 
...
- **Success:** If we can source data based on just the phone number, the [response body](/reference/api/endpoints#post-1-click-response) will contain a [`1ClickEntity`](/reference/api/types#1clickentity) (see this [example](/reference/api/types#example-1clickentity)), keyed on `identity`, that contains the user's data and metadata about it. <br/> → _Jump to [step 5](#5-prompt-user-to-confirm-their-data)._
- **Error:** Otherwise, the response body will contain an [error](/reference/api/errors) with a Verified error code:
  - **[`OCE011`](/reference/api/errors#oce011)** indicates that we need additional information (the user's birth date and/or SSN4) to source data. <br/> → _Continue to [step 3](#3-if-necessary-prompt-user-for-additional-info)_.
  - **[`OCE013`](/reference/api/errors#oce013)** indicates that Verified found no [credentials](/reference/data/outputs#credentials) for the user. <br/> → _Fall back to a manual signup flow._
  - **[`OCE017`](/reference/api/errors#oce017)** indicates that Verified blocked the user because the risk score was too high. <br/> → _Reject the user or flag them and fall back to a manual signup flow._

:::tip Data is Retrievable for 60 Minutes
**You can retrieve data for 60 minutes,** at which point it's deleted. If you need to retrieve the same data again, use [`GET /1-click`](/reference/api/endpoints#get-1-click) instead of `POST /1-click` so that a duplicate 1-Click Signup event isn't created (and so you aren't billed twice).
:::
...
<PromptUserToConfirmTheir1ClickSignupData />
