...
<TabItem value="free" label="Free Only">
  ...
  **You must obtain user consent before issuing credentials.** Not doing so violates our terms and will prevent us from paying you for issuing credentials. 
  ...
  Format the credential data according to the `properties` attribute of the credential schema. In this case, the schema is for a `SsnCredential` which can found found at [`https://schema-resolver.verified.inc/jsonSchema/SsnCredential`](https://schema-resolver.verified.inc/jsonSchema/SsnCredential). 
