...
> A **credential** is a collection of data about a person. It's issued by a company and can be requested by other network partcipants, gated by the user's consent.
...
**Components:** A company issues credentials using the [/credentials](/api-overview#issue-credentials) API endpoint. The data is store securely using a data privacy vault and is only avaliable to the user and brands the user agrees to share with.
...
The client is then expected to direct to the url, which will be to <a href="/web-wallet">Unum ID Web Wallet</a>, where the user will be required to authenticate then agree to share the data relevant data.

At a high level, to create a request a company inputs a list containing three pieces of information for each credential:
1. type of the credential
2. acceptable <Tip type="brand">brand(s)</Tip> of the credential
...
> A **user** is an individual in the Unum ID network that can share credential data. Each user has at least one phone or emails associated with them. They can have multiple of either.

:::note
To avoid having to define, query, and share the Unum ID User "ID", users can be simply referenced by email and/or phone, identifiers that your already have for your own user accounts.
:::
...
**Components:** Referenced in API endpoints [/hasMatchingCredentials](/api-overview#check-user-credentials) and [/credentials](/api-overview#issue-credentials). User data is associated by using these user identifiers that you already keep on your users.
...
> A **brand** is a company entity that has a corresponding unique api key, name, and card image. Brands can issue, request and receive <a href="/terminology#credential">credentials</a> to and from <a href="/terminology#user">users</a>.
...
**Components:** Each brand has an associated parent customer. It is totally okay if your customer only has one brand. We want to have the flexibility to support multiple brands per customer.