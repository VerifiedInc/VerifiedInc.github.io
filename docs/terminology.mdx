...
- Hooli verifies Richard's address and issues a credential of type [`AddressCredential`](/schema#schema-library) to him (as part of [Issue to Earn](/issue-to-earn)).
- Kredita <a href="#request"><Tip type="request">requests</Tip></a> an address credential from Richard (as part of [1-Click Signup](/1-click-signup)).
...
The data must match the [data schema](/schema) for the credential type.


### Atomic

Atomic credentials contain single data points. For example, an [`EmailCredential`](/schema#schema-library) is an atomic credential that contains an email address.

Some atomic credentials can be contained within [composite](#composite) credentials. For example, [`FirstNameCredential`](/schema#schema-library) is an atomic credential that can be contained within the composite [`FullNameCredential`](/schema#schema-library).
...
Composite credentials are composed of multiple atomic credentials. For example, a [`FullNameCredential`](/schema#schema-library) can be composed of a [`FirstNameCredential`](/schema#schema-library) and a [`LastNameCredential`](/schema#schema-library). It can also include a [`MiddleNameCredential`](/schema#schema-library).
