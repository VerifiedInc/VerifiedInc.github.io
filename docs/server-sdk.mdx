...
### Setup
One needs to provide the SDK the the environment it is running in, i.e. Production. For the Typescript SDK this done via the NODE_ENV environment variable. The three relevant values are: `Production`, `Staging`, `Dev`. You need to use one of these exactly in order for the SDK to communicate with Unum ID's SaaS. 
