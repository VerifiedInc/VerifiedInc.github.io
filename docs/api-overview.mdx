...
- `PhoneCredential` is a "[single attribute atomic credential](/schema#schema-definitions)" and therefore does not have any sub-credentials. Because we specified a `allowUserInput` attribute, user will be allowed to prompt its own information for the given credential."
