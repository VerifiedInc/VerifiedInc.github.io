...
> Easiest way to get <Tip type="user">user</Tip>'s <Tip type="credential">credentials</Tip> with their consent to onboard to your service.
>>>>>>> dev
...
1. User's  phone
<<<<<<< HEAD
2. (Optional) Verification messaging option. _Controls the SMS verification messaging._
3. (Optional) Content object of request page options. _To allow for minor messaging adjustments to the Web Wallet request page the user provides consent to share data from._
4. (Optional) Credential request(s). _If you would like to update from the default credential requests._
=======
2. (Optional) Credential request(s). _If you would like to update from the default credential requests._
3. (Optional) Verification messaging option. _Controls the SMS verification messaging._
4. (Optional) Content object of request page options. _To allow for minor messaging adjustments to the Web Wallet request page the user provides consent to share data from._
>>>>>>> dev
...
:::note
This endpoint performs a similar role to [/hasMatchingCredentials](/api-overview#check-user-credentials) but with a few key differences:
- Can only be used with a user's phone number
- Triggers an SMS verification message to the user that Verified Inc sends
- There is a much greater chance of success getting user data than the standard acceptance flow thanks to data pulled on demand
:::

:::caution
The same ApiKey necessary to call [/1-click](/api-overview#1-click-onboarding) grants you and only you access to the encrypted credential data documented in [Get Shared Credentials](#get-shared-credentials). Please keep this key secure.
:::
...
=======
  credentialRequests?: CredentialRequest[], // Optional: a list of one or more CredentialRequest objects. Encodes which credentials are being asked for
  verificationOption?: 'both_link_and_code' | 'only_link' | 'only_code' // Optional: controls the SMS verification messaging. Default is 'both_link_and_code'.
  content?: {
    title?: 'Signup' | 'Verify' | 'Apply', // Optional: title of the request page. Default is 'Signup'
    description?: string, // Optional: description displayed under the consent page header. Default is 'Please make sure everything is correct:'
  }
>>>>>>> dev
}
```
