...
You need to be able to receive these credential requests from Unum ID and pass them to this function. To do this, you need to create a `/credentialRequest` endpoint that conforms to our [OpenAPI specification](https://gist.github.com/UnumIDAdmin/d76d9fe46e459e529d7f7b6f9319a0b6). 
...
:::note
Despite this having "verify" in its name, this function only serves Issuers in determining whether a subject's request for credentials is valid. It is up to you application logic to determine whether you have the data relating to the the subject to issue the requested credentials.
:::

```typescript title="SubjectCredentialRequest"
export interface SubjectCredentialRequest {
  ...
  proof: Proof; // Cryptographic proof information
}
```
...
{
  "authToken": string, // your auth token
  "issuerDid": string, // did of your issuer
  "credentialRequests": SubjectCredentialRequest[] // list of signed credential requests. This ought to be provided verbatim via the spec'd /credentialRequests endpoint.
}
```

:::important
The SubjectCredentialRequest array ought to be able to be passed as is from the specified OpenApi [/credentialRequest](https://gist.github.com/UnumIDAdmin/d76d9fe46e459e529d7f7b6f9319a0b6) endpoint.
:::

#### Response Body: [`SubjectCredentialRequestVerifiedStatus`](https://docs.unumid.co/server-sdk-typescript/interfaces/subjectcredentialrequestverifiedstatus.html)
```typescript title="SubjectCredentialRequestVerifiedStatus"
{
...
The `status` attribute of the response is of type [CredentialStatusOptions](https://docs.unumid.co/types/modules.html#credentialstatusoptions). Currently the only valid status are: verified and revoked.
...
#### Response Body: [`CredentialStatusInfo`](https://docs.unumid.co/server-sdk-typescript/interfaces/credentialstatusinfo.html).
