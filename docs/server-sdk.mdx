...
**Before you begin:** Please read the [deployment overview](deployment-overview#server-sdk). You need to be registered as an Verified Inc. customer, and you need to register at least one issuer or verifier. (You can register zero to many of each, depending on your use case.) You'll receive issuer and verifier API keys to use with this SDK.

:::note
Auto generated SDK documentation can be found [here](https://docs.verified.inc/server-sdk-typescript/index.html). Similar documentation for object definitions can be found [here](https://docs.verified.inc/types/index.html). The auto generated documentation is more granular then this page, however less styled.
:::
...
### Errors
Errors returned by the Verified Inc. cloud will also be wrapped in the `UnumDto` object so that the potentially updated `authToken` can be retrieved. Validation errors, which are created prior to any calls to Verified Inc., will be of type `Error` and are thrown. Because in this case a network call is never made, there's no potential new `authToken` to pass back. For this reason, **we recommend wrapping all SDK calls in a try/catch.**
...
#### UNUM_ENV
One needs to provide the SDK the with an environment variable to denote its run time environment, i.e. `production`. For the Typescript SDK this done via the `UNUM_ENV` environment variable. The three relevant values are: `production`, `sandbox`, `dev`. You need to use one of these exactly in order for the SDK to communicate with Verified Inc.'s SaaS. The default value if nothing is supplied is `sandbox`.
...
The optional [VersionInfo](https://docs.verified.inc/types/interfaces/versioninfo.html) attribute is where request header requirements are defined in order to use your API. It is also useful for how a <a href="/terminology#subject"><Tip type="subject"/></a>'s [CredentialRequests](https://docs.verified.inc/types/interfaces/subjectcredentialrequests.html) should be routed to your application in order for the corresponding Server SDK version to preform verifications. This can be updated at later point from the Verified Inc. [dashboard](https://dashboard.sandbox-unum.id/#/login). If not supplied simply the `url` attribute will be used.
...
#### Response Body: [`RegisteredIssuer`](https://docs.verified.inc/server-sdk-typescript/interfaces/registeredissuer.html)
...
#### Response Body: [`Credential`](https://docs.verified.inc/types/interfaces/credentialpb.html) Array
...
:::important
The private keys *never* leave your server. The Server SDK only needs it to perform cryptographic functions on your behalf.
:::
...
#### Response Body: [`VerifiedStatus`](https://docs.verified.inc/server-sdk-typescript/interfaces/verifiedstatus.html)
...
You need to be able to receive these credential requests from Verified Inc. and pass them to this function. To do this, you need to create a `/userCredentialRequests` endpoint that conforms to our [OpenAPI specification](https://gist.github.com/UnumIDAdmin/fd75b174b33ff0c5b4de87118cb11188). 
...
The main use case of this to allow bootstrapping users that just installed the Verified Inc. Wallet with credentials necessary to use across the network, i.e for instant sign ups with a partner. 
...
The optional [VersionInfo](https://docs.verified.inc/types/interfaces/versioninfo.html) attribute is where request header requirements are defined in order to use your API. It is also useful for how encrypted <a href="/terminology#presentation"><Tip type="presentation"/></a>s from Verified Inc.'s SaaS, which originate from the Mobile SDK, should be routed to your application in order for the corresponding Server SDK version to preform verifications. This can be updated at later point from the Verified Inc. [dashboard](https://dashboard.sandbox-unum.id/#/login). If not supplied simply the `url` attribute will be used. 
...
#### Response Body: [`RegisteredVerifier`](https://docs.verified.inc/server-sdk-typescript/interfaces/registeredverifier.html)
...
You need to be able to receive presentations from users and pass them to this function. To do this, you need to create a `/presentation` endpoint that conforms to our [OpenAPI specification](https://gist.github.com/UnumIDAdmin/a81ffe5bd383b1ec36cb1b6bb1ccd26e). The Verified Inc. cloud sends encrypted presentations to this endpoint, which should pass those presentations to the `verifyPresentation` function to be decrypted and verified.
...
:::important
Verified Inc. *never* has access to sensitive data contained in presentations. The [Mobile SDK](mobile-sdk-overview) encrypts each presentation (with your verifier encryption public key) before sending it to us. We have no ability to decrypt the data — only you do.
:::
...
Messages you send will (necessarily) be delivered from an Verified Inc. associated phone number, but this association won't be visible to the user. You can of course use your own SMS sending service if you prefer.
...
Emails you send will be delivered from no-reply@verified.inc. You can of course use your own email sending service if you prefer.
...
The `status` attribute of the response's [CredentialStatusInfo](https://docs.verified.inc/types/interfaces/credentialstatusinfo.html) is of type [CredentialStatusOptions](https://docs.verified.inc/types/modules.html#credentialstatusoptions). Currently the only valid status are: verified and revoked.
...
#### Response Body: [`CredentialIdToStatusMap`](https://docs.verified.inc/types/interfaces/credentialidtostatusmap.html).
